...
和绘制/拖动预览属于活动层，最后以 RAW 几何直接绘制。面积被选中时，被动层
保留普通填充和标签，活动层只补充加粗轮廓和控制点，既不会覆盖标签，也不会
改变半透明面积之间的文档层叠顺序。真正开始编辑或拖动时，才精确
恢复旧主体覆盖的局部背景，再绘制预览，因此不会保留旧轮廓残影。缺少图块时也
立即使用当前缩放级别的精确直接渲染，不拉伸旧缩放图块、不显示模糊占位图。
...
| 后台图块快照 | 128MiB | 对所有在途请求做保守估算；超预算拒绝入队并继续走精确直接渲染 |
...
短暂突破 128MiB。基准 JSON 的 `overlay_cache.tiles.pending_bytes` 及
...
`pending_bytes` 和直接渲染路径判断后台压力。任何丢弃都不能解释为测量或
画面丢失，精确直接路径仍应保证内容完整。
