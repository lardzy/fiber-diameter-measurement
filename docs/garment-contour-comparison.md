...
1. 分别点击“处理前”和“处理后”的“导入照片”。下拉菜单也可选用主窗口已打开的普通图片。导入后自动识别物体，彩色覆盖表示参与计算的范围。
...
4. 点击“标定”，在与衣物同平面的标尺上点两端，再输入实际长度（mm）。固定机位且图像尺寸相同时，默认共用中线与标定，只需设置一次。不同像素尺寸会关闭共用，需分别设置。
5. 打开“变化结果”。默认以处理前照片为底图，叠加前后轮廓。点击样品或表格行，紫色截线、蓝色圆点（前）、橙色方点（后）和下方左右外缘数值同步定位。可用“上一处／下一处”或参考高度连续查看；“详细数据”展开前后距离等列。修正后自动重算。
...
| 补入轮廓 | 保留现有轮廓，补入漏选区域 |
| 剔除区域 | 保留现有轮廓，扣除点选到的区域；“排除背景”表示本次不应剔除的部分 |
