...
- **标定**：图内比例尺标定、标定预设、项目统一比例尺和 CU 标尺导入。标定单位支持纳米（nm）、微米（μm）、毫米（mm）、厘米（cm）和米（m），普通图片与数字切片均可使用；长度、面积和比例尺随标定单位显示，面积使用对应平方单位。[单位说明](docs/measurement-units.md)。
...
- **画布比例尺**：点击右下角“对象捕捉”旁的“比例尺”即可预览、拖动和调整长度／样式，再直接导出。提供六种样式，默认红色端点刻度、右下角；显示与导出范围独立，记住最近确认的设置，重启默认隐藏。详见 [比例尺画布编辑](docs/scale-overlay-editing.md)。
