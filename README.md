...
分析结果可独立导出为审计型 Excel 工作簿、单表/曲线 CSV，或包含工作簿、关联资产和清单的便携 ZIP 包。批量高级分析使用内置的平面分析配方，当前覆盖方向性、Tubeness、GLCM、二维强度表面和“方向性 + GLCM”组合。
...
- `.fdmslide` 多焦层浏览、缩放、焦层切换、平滑移动和另存压缩副本。
...
“工具 → 截图工具”可以启动独立的 `FiberScreenshotTool` 伴随进程。它在系统托盘中常驻，关闭测量工作台不会自动结束；总开关、Windows 登录自启、输出目录、文件名模板、PNG/JPEG/WebP、保存与剪贴板任务、延时、鼠标指针、编辑器以及各模式的全局快捷键均保存在独立的 `screenshot-settings.json` 中。

当前截图模式包括区域选择、智能窗口/子窗口、活动窗口、当前显示器、全部屏幕、上次区域，以及 CU-5 实时预览。区域选择支持拖动；智能模式会预选鼠标下的原生窗口或控件，并可用 Tab、PageUp/PageDown 或滚轮切换层级。标注编辑器提供矩形、椭圆、箭头、线、画笔、文字、编号、高亮、马赛克、模糊、裁剪和撤销/重做，点击“完成”后才按统一的保存/剪贴板设置输出。

CU-5 专用模式不会启动第二个 Microview 实例，也不会访问或抢占采集卡。它只枚举已经运行的 `CU-5.exe` 原生子窗口，并结合 `CWndForSDK`、MFC/MDI 层级、控件标识和 4:3 视频区域特征，自动裁出“实时预览”里的视频画面而不是整个 CU-5 窗口。首次使用建议在截图设置页执行“检测 CU-5 实时预览区域”。为避免静默截错，候选不明确、窗口隐藏/最小化或画面捕获质量异常时会直接报告失败。

旧版 CU-5/Microview 可能使用 DirectDraw 或硬件叠加。当前 Windows 捕获链会尝试窗口捕获，并在失败或黑帧时回退到可见桌面区域；因此 CU-5 窗口应保持可见且未最小化。不同采集卡、125%/150% DPI、窗口遮挡和驱动组合仍需在目标 Windows 设备上实机确认。
...
- 数字化切片图片导出只处理当前焦层的当前原始像素视口；批量高级分析必须显式冻结当前视口，不会扫描整片。
